...
@pytest.mark.asyncio
async def test_e2e_search_and_get_details(mock_search_html, mock_track_details_html):
    """Test complete flow: search track and get details."""
    with patch.object(WhoSampledScraper, '_fetch_page', new_callable=AsyncMock) as mock_fetch:
        # Setup mock responses
        mock_fetch.side_effect = [
            mock_search_html,  # For search_track call
            mock_search_html,  # For get_track_samples search call
            mock_track_details_html  # For get_track_samples details call
        ]
...
@pytest.mark.asyncio
async def test_e2e_direct_url_lookup(mock_track_details_html):
...
@pytest.mark.asyncio
async def test_e2e_track_with_covers_and_remixes(mock_search_html, mock_track_details_html):
    """Test flow for track with covers and remixes."""
    with patch.object(WhoSampledScraper, '_fetch_page', new_callable=AsyncMock) as mock_fetch:
        mock_fetch.side_effect = [mock_search_html, mock_track_details_html]
...
@pytest.mark.asyncio
async def test_e2e_multiple_searches_sequential(mock_search_html):
...
@pytest.mark.asyncio
async def test_e2e_error_handling_http_error():
    ...
    with patch.object(WhoSampledScraper, '_fetch_page', new_callable=AsyncMock) as mock_fetch:
        ...
        assert len(result) == 1
        assert "Error" in result[0].text or len(result[0].text) > 0


@pytest.mark.asyncio
async def test_e2e_with_youtube_links(mock_search_html, mock_track_details_html):
...
@pytest.mark.asyncio
async def test_e2e_empty_track(mock_empty_track_html):
    """Test flow for track with no samples/covers/remixes."""
    search_html = """
    <html>
        <body>
            <div class="topResult">
                <a class="trackTitle" href="/Some/Obscure-Track/">Some Obscure Track</a>
    ...
    with patch.object(WhoSampledScraper, '_fetch_page', new_callable=AsyncMock) as mock_fetch:
        mock_fetch.side_effect = [search_html, mock_empty_track_html]
